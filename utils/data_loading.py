...
def create_output_data_cache_filepath(subreddit, date_range, model_alias, num_topics):
    """
    ...
    num_topics: int or None
        number of topics created by the model
    ...
    """
    filename = str(subreddit) + '&' + str(date_range[0]) + '_' + str(date_range[1]) + '&'+ str(model_alias) + '&'+ str(num_topics) + '.obj'
    return os.path.join(d.CACHE_DIR, filename)

def check_cache_existance(subreddit, date_range, model_alias, num_topics):
    """
    ...
    return not cached_df.loc[(cached_df['subreddit'] == subreddit) & (cached_df['date_range_0'] == date_range[0]) & (cached_df['date_range_1'] == date_range[1]) & (cached_df['model'] == model_alias) & (cached_df['num_topics'] == num_topics), :].empty
